...
export default function LoginPage() {
  ...
  // Mensaje informativo cuando se llega al login por cierre de sesión automático
  useEffect(() => {
    ...
    <div className="min-h-screen flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 bg-ink-950 text-ink-100 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-ink-900/90 backdrop-blur-xl border border-ink-800 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-400 to-brand-600 text-ink-950 flex items-center justify-center mx-auto shadow-lg shadow-brand-500/25">
        ...
        <div className="p-3.5 rounded-2xl bg-ink-800/60 border border-ink-700/60 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-brand-400/15 hover:bg-brand-400/25 text-brand-200 border border-brand-400/30 text-xs font-semibold transition-all hover:scale-[1.02]"
            >
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-brand-400 hover:bg-brand-300 text-ink-950 font-semibold text-sm shadow-lg shadow-brand-500/25 transition-all hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50"
          >
            {loading ? (
